...
Author: Mark Flores
Purpose: 
The purpose of this .geo file is to script a bolted single lapjoint. 
...
For i In {0:(nply_bolt-1)}
	...
	x1 = a/2; y1 = b/2; 
	// Hole In Plate!!
	x = D*sf/2-tol; y = D*sf/2-tol; z = 3*T/2; lc = lc_S;
	...
	Extrude {0,0,-4*T} { Surface{s41[i]}; Layers{nep_bolt}; Recombine;}
EndFor
...
For i In {0:(nply_bolt-1)}
	...
	x1 = a/2; y1 = b/2; 
	// Hole In Plate!!
	x = D*sf/2+tol; y = D*sf/2+tol; z = T+tol; lc = lc_S;
