...
x=L; y=0; z=W/2; lc=lc_S;
tol=0.001; 
y=0+tol;
For i In {0:(nply-1)}
  p31[{i}]=newp; Point(p31[i]) = { x, y-tply*i,-z, lc};
  p32[{i}]=newp; Point(p32[i]) = {-x, y-tply*i,-z, lc};	
  p33[{i}]=newp; Point(p33[i]) = {-x, y-tply*i, z, lc};	
  p34[{i}]=newp; Point(p34[i]) = { x, y-tply*i, z, lc};	
  l31[{i}]=newl;  Line(l31[i]) = {p31[i],p32[i]};
  l32[{i}]=newl;  Line(l32[i]) = {p32[i],p33[i]};
  l33[{i}]=newl;  Line(l33[i]) = {p33[i],p34[i]};
  l34[{i}]=newl;  Line(l34[i]) = {p34[i],p31[i]};
  ll31[{i}]=newll; Line Loop(ll31[i]) = {l31[i],l32[i],l33[i],l34[i]}; 
  s31[{i}]=news; Plane Surface(s31[i]) = {ll31[i]};
  Extrude {0,-tply,0} {Surface{s31[i]}; Layers{nep}; Recombine;}
EndFor
