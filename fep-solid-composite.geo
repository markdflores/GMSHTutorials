/*
Author: Mark Flores
Purpose: Create a hole in a plate with a given width, length, and thickness. 
The program allows you to insert the hole with a given diameter anywhere in 
the plate’s boundary.  The program does not work if you put the hole on the edges. 
The program also allows you to fill in the hole with a laminated composites or a 
solid. The filler is a laminated composite where the out-of-plane direction aligns with 
...
//
// - - - - Filled Elastomer is One Solid Instead of Composites
//
...
x = (D/2-tol)*s; y = (D/2-tol)*s; z = T/2-tply*i; lc = lc_R;
...
Field[1] = Box; 
Field[1].VIn = lc_R/mesh_refine_lc;  
Field[1].VOut= lc_S; 
