...
// - - - - - - - - - - - - - - - - - - - - - - - 
asym = 0; // 1) Yes not explored yet. . . 
numply = nply/2; // Stop at mid plane of composite
// numply = 1; // Stops at first ply of composite. 
// - - - - - - - - - - - - - - - - - - - - - - -
...
For i In {0:(nnC-2)}	// For Loop - Curvature
  y=R1*s*(nnC-i-1)/nnC; x=x0-Sqrt(R1^2-(y-y0)^2);
...
l010=newl;BSpline(l010) = {p010,p014[],p018};	// Cartesian Quadrant II
l011=newl;BSpline(l011) = {p011,p015[],p018};	// Cartesian Quadrant III
l012=newl;BSpline(l012) = {p012,p016[],p017};	// Cartesian Quadrant IV
l013=newl;   Line(l013) = {p001,p002}; 		// Left Line
l014=newl;   Line(l014) = {p003,p004}; 		// Right Line
// - - - - - Simplify Lines - - - - 
l01[] = {l013};
l02[] = {l002,l006,l010,-l011,-l007,-l003};
l03[] = {l014};
l04[] = {-l001,-l005,-l009,l012,l008,l004};
// - - - - - Line Loop - - - - - 
...
nnHy = 2; 	// Number of Holes in Y
l=0;
For i In {0:(nnHx-1)}
  For j In {0:(nnHy-1)}
  l=j+nnHy*i;  
  // - - - - Center of Pin Holes - - - - 
  x=(nnHx*i-1)*W_pin/2; y=(nnHy*j-1)*L_pin/2; z=T/2; lc=lc_S;
...
For i In {0:(nply-1)}
  ...
  bnd[]={numv[3]:numv[8]};   bot[{i}]=newreg; Physical Surface(bot[i])=bnd[]; 
  bnd[]={numv[9]}; 	     left[{i}]=newreg; Physical Surface(left[i])=bnd[]; 
  bnd[]={numv[10]:numv[15]}; right[{i}]=newreg; Physical Surface(right[i])=bnd[];
  For j In {0:l}
  ...
  bnd[]={numv[24]:numv[27]}; pinh3[{k}]=newreg; Physical Surface( pinh3[k])=bnd[];
...
For i In {0:(nnHx-1)}
  ...
  x=(nnHx*i-1)*W_pin/2; y=(nnHy*j-1)*L_pin/2; rd = R_pin*s-tol_pin; 
...
// - - - - - - - - - - - - - - - - - - - - - - -
mesh_reduce = 4; 	// Reduce
mesh_radius = R_pin*2; 	// Radius of Cylinder
...
For i In {0:(nnHx-1)}
  ...
  xc = (nnHx*i-1)*W_pin/2;	// Center of Cylinder w/Respect to X 
  yc = (nnHy*j-1)*L_pin/2;  		// Center of Cylinder w/Respect to Y
